...
export default function SatelliteBatchesPage() {
 ...
 setTimeout(() => setToastMsg(""), 3000);
  ...
  const isAdminOrManager = roleUpper === "01" || roleUpper === "02" || roleUpper === "ADMIN" || roleUpper === "QUẢN LÝ CÔNG VIỆC";
 ...
 const handleAssignBatchToStaff = async () => {
   if (!selectedBatch || isAssigning) return;
 ...
 try {
   ...
   if (res.ok) {
     ...
     if (data.success) {
        ...
        triggerToast(data.error || "Lỗi khi xóa lô mail!");
     }
   } else {
     const errData = await res.json().catch(() => ({}));
     triggerToast(errData.error || "Lỗi kết nối máy chủ khi xóa lô!");
   }
 } catch (err) {
   console.error("Lỗi xóa lô mail:", err);
   triggerToast("Lỗi hệ thống!");
 } finally {
