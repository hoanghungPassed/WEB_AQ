...
function MailTableContent() {
  ...
  const getChannelStatusColor = (status: string) => {
    ...
    return "text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-white/5";
  };
    ...
    <div className="h-[calc(100vh-120px)] flex flex-col space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900 dark:text-white tracking-tighter flex items-center gap-2 uppercase">
      ...
      {/* Grid Table Area */}
      <div className="flex-1 min-h-0 rounded-2xl border border-border-custom bg-white dark:bg-sidebar flex flex-col shadow-2xl overflow-hidden">
        <div className="flex-1 overflow-auto custom-scrollbar">
          <table className="w-full text-left border-collapse min-w-[1200px]">
            <thead className="sticky top-0 z-20 bg-white dark:bg-[#121212]">
              <tr className="bg-gray-100 dark:bg-white/5 border-b border-border-custom shadow-sm">
                ...
                <tr key={mail.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors group animate-fade-in">
                  ...
                  >
                    <div className="flex items-center justify-between gap-1 overflow-hidden">
                      <span className="text-gray-900 dark:text-white font-medium text-[11px] truncate group-hover:text-gold transition-colors">{mail.email}</span>
                  ...
                  <td 
                    ...
                    className="px-3 py-2 text-[11px] text-gray-600 dark:text-gray-400 font-mono border-r border-border-custom cursor-pointer active:bg-gold/10 hover:text-gray-900 dark:hover:text-gray-900 dark:text-white truncate whitespace-nowrap"
                  >
                    {mail.pass}
                  </td>
                  <td 
                    onClick={() => copyToClipboard(mail.recovery || "")}
                    className="px-3 py-2 text-[11px] text-gray-600 dark:text-gray-400 border-r border-border-custom cursor-pointer active:bg-gold/10 hover:text-gray-900 dark:hover:text-gray-900 dark:text-white truncate whitespace-nowrap"
                  >
                    {mail.recovery}
                  </td>
                  <td 
                  ...
                  <td className="px-3 py-2 text-center whitespace-nowrap">
                    <a 
                      href={mail.otpLink} 
                      target="_blank" 
                      className="text-gold hover:text-gray-900 dark:hover:text-gray-900 dark:text-white transition-colors flex justify-center whitespace-nowrap"
                    >
        ...
        {/* Compact Pagination */}
        <div className="px-6 py-3 border-t border-border-custom flex items-center justify-between bg-gray-50 dark:bg-white/[0.02] flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-1.5 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5 text-gray-900 dark:text-white disabled:opacity-30 hover:bg-gold/20 transition-all"
            >
              <ChevronLeft size={16} />
            </button>
            <button 
...
export default function MailManagementPage() {
  return (
    <Suspense fallback={<div className="text-gray-900 dark:text-white">Đang tải dữ liệu...</div>}>
      <MailTableContent />
    </Suspense>
  );
}
