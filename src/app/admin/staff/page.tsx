...
function StaffManagementContent() {
  ...
  const handleResolveAppeal = async (action: "APPROVE" | "REJECT", newStatus: string) => {
    if (!activeDetailDay?.dbRecord?._id || resolvingAppeal) return;
    setResolvingAppeal(true);
    try {
      const res = await fetch("/api/admin/attendance/resolve-appeal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          attendanceId: activeDetailDay.dbRecord._id,
          action,
          newStatus
        })
      });
      ...
      if (res.ok && data.success) {
        setToastMsg(action === "APPROVE" ? "Đã duyệt khiếu nại thành công!" : "Đã từ chối khiếu nại!");
        setShowToast(true);
        ...
        setToastMsg(data.error || "Xử lý khiếu nại thất bại");
        setShowToast(true);
        setTimeout(() => setShowToast(false), 3000);
      }
    } catch (err) {
      console.error(err);
      setToastMsg("Lỗi kết nối khi gửi yêu cầu");
      setShowToast(true);
      setTimeout(() => setShowToast(false), 3000);
    } finally {
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-60 overflow-y-auto custom-scrollbar p-2 bg-black/20 rounded-2xl border border-white/0">
                  {(staffList || []).filter(s => s.status === "ACTIVE").map(staff => (
                    <div
                      ...
                      className={`p-3 rounded-xl border cursor-pointer transition-all flex items-center gap-3 ${dutySelectedStaff.includes(staff.id) ? "bg-gold/10 border-white/0" : " bg-white/5 border-transparent hover:bg-white/10"
                        }`}
                    >
                      <div className={`h-4 w-4 rounded-[4px] border flex items-center justify-center transition-colors ${dutySelectedStaff.includes(staff.id) ? "bg-gold border-gold" : "border-gray-500"
                        }`}>
                  ...
                  {activeTab === "PENDING" && pendingSubTab === "ACCESS" ? (
                    ...
                    (currentStaff || []).length > 0 ? (currentStaff || []).map((staff) => (
                      <tr key={`${staff.id}-${staff.username}`} className="group hover:bg-zinc-800 bg-zinc-900/[0.02] transition-all cursor-pointer" onClick={() => setSelectedStaff(staff)}>
                        ...
                        <td className="px-8 py-7">
                          {activeTab === "ACTIVE" ? (
                            <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black tracking-widest uppercase border whitespace-nowrap ${staff.role === "01" ? "bg-red-500/10 text-red-500 border-red-500/20" :
                        ...
                        {!isRestricted && (
                          <td className="px-8 py-7">
                            {activeTab === "ACTIVE" ? (
                              <span className={`px-3 py-1 rounded-xl text-[10px] font-bold tracking-wider uppercase border whitespace-nowrap ${staff.status === "ACTIVE" ? "bg-green-500/10 text-green-500 border-green-500/20" : "bg-red-500/10 text-red-500 border-red-500/20"
                                }`}>
                  ...
                  <div className="flex items-center gap-1">
                    {[...Array(totalPages)].map((_, i) => (
                      <button
                        ...
                        className={`h-10 w-10 rounded-xl border transition-all text-[10px] font-black ${currentPage === i + 1
                    ...
                    {selectedStaff.status === "PENDING" ? (
                      ...
                      <div className="grid grid-cols-2 gap-4 w-full">
                        ...
                        <button
                          ...
                          className={`h-14 rounded-2xl border font-bold uppercase text-[10px] tracking-wider transition-all flex flex-col items-center justify-center gap-1 ${selectedStaff.status === "ACTIVE"
                  ...
                  <div className="space-y-4 pt-4 border-t border-white/0">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3 bg-black/20 border border-white/0 rounded-3xl p-5">
                      {(attendanceData.list || []).map((day) => {
                        ...
                        if (day.dbRecord?.complainStatus === "PENDING") {
                          borderStyle = "animate-pulse bg-amber-500/20 border-amber-500 text-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.4)]";
                          statusColor = "text-amber-400";
                        } else if (day.dbRecord?.complainStatus === "RESOLVED") {
                          borderStyle = "bg-green-500/10 border-green-500/30 text-green-400";
                        } else if (day.dbRecord?.complainStatus === "REJECTED") {
                          borderStyle = "bg-red-500/10 border-red-500/30 text-red-400";
                        }
        ...
        {activeDetailDay && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4">
            ...
            >
              ...
              {/* Status Header */}
              <div className={`p-6 rounded-[28px] border mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${activeDetailDay.hasCheckedIn
              ...
              {/* Appeal Section for Admin */}
              {activeDetailDay.dbRecord?.complainStatus && (
                <div className="mt-8 p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4 text-left">
                  <div className="flex justify-between items-center text-sm font-bold uppercase">
                    <span className="text-gray-400">Khiếu nại chuyên cần:</span>
                    <span className={
                  ...
                  <div className="text-sm text-gray-300 leading-relaxed">
                    <span className="font-bold text-gray-400 block uppercase mb-1 text-xs">Lý do khiếu nại của nhân viên:</span>
                    {activeDetailDay.dbRecord.complainText}
                  </div>

                  {activeDetailDay.dbRecord.complainStatus === "PENDING" && (
                    <div className="pt-2 flex flex-col sm:flex-row gap-3">
                      <button
                        onClick={() => handleResolveAppeal("APPROVE", "Đúng giờ")}
                        disabled={resolvingAppeal}
                        className="flex-1 h-12 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white font-bold uppercase text-xs tracking-wider rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer"
                      >
                        {resolvingAppeal ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} Duyệt Có Mặt
                      </button>
                      <button
                        onClick={() => handleResolveAppeal("REJECT", "Vắng mặt")}
                        disabled={resolvingAppeal}
                        className="flex-1 h-12 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-bold uppercase text-xs tracking-wider rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer"
                      >
                        {resolvingAppeal ? <Loader2 size={14} className="animate-spin" /> : <XCircle size={14} />} Từ Chối
                      </button>
                    </div>
                  )}
                </div>
